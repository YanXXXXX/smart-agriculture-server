...
public class TaskEmployeeServiceImpl implements ITaskEmployeeService {
    ...
    @Transactional
    @Override
    public int insertTaskEmployeeByTaskId(TaskEmployee taskEmployee) {
        taskEmployeeMapper.deleteTaskEmployeeByTaskId(taskEmployee.getTaskId());
        if(taskEmployee.getEmployeeIds().length > 0){
            for (Long employeeId : taskEmployee.getEmployeeIds()) {
                ...
                taskEmployeeMapper.insertTaskEmployee(taskEmployee);
            }
        }
        return 1;
    }
    ...
    public int deleteTaskEmployeeByTaskIdAndEmployeeId(Long taskId, Long employeeId) {
        return taskEmployeeMapper.deleteTaskEmployeeByTaskIdAndEmployeeId(taskId, employeeId);
    }
}
